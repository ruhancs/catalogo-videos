...
describe('CastMemberSequelizeRepository Integration Tests', () => {
  ...
  it('should insert a cast-member', async () => {
    const item = CastMember.fake().anActor().build();

    await repository.insert(item);
    const itemModel = await CastMemberModel.findByPk(item.cast_member_id.id);
    expect(itemModel!.toJSON()).toStrictEqual({
      cast_member_id: item.cast_member_id.id,
      name: item.name,
      type: item.type.type,
  ...
  it('should bulk insert the cast members', async () => {
    const item = CastMember.fake().anActor().build();
    const item2 = CastMember.fake().anActor().build();

    await repository.bulkInsert([item, item2]);
    const itensModel = await CastMemberModel.findAll();
    expect(itensModel).toHaveLength(2);
  });

  it('should find a cast-member or cast-members by id', async () => {
    const item = CastMember.fake().anActor().build();
    const item2 = CastMember.fake().anActor().build();

    await repository.insert(item);
    await repository.insert(item2);
    const itemFounded = await repository.findById(item.cast_member_id);
    const itensFounded = await repository.findByIds([
      item.cast_member_id,
      item2.cast_member_id,
    ]);

    expect(itemFounded).toStrictEqual(item);
    expect(itensFounded).toHaveLength(2);
  });
