...
export class CastMemberFakeBuilder<TBuild = any> {
  ...
  static theActors(countObjs: number) {
    //construto de CastMemberFakeBuilder recebe a quantidade que deve ser criado
    return new CastMemberFakeBuilder<CastMember[]>(countObjs).withType(
      //value object CastMemberType metodo statico para criar ator
      CastMemberType.createAnActor(),
    );
  }
  ...
  build(): TBuild {
    const castMembers = new Array(this.countObjs) // cria um array com a
      .fill(undefined) // preenche os valores do array com undefined
      .map((_, index) => {
