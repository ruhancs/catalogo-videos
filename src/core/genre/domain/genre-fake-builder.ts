...
export class GenreFakeBuilder<TBuild = any> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private _categories_id: PropOrFactory<CategoryId>[] = []; //map de ids de categorias gerado no build
  ...
  build(): TBuild {
    const Genres = new Array(this.countObjs).fill(undefined).map((_, index) => {
      ...
      //verificar quantos item tem em this._categories_id cria a quantidade de categories id com o index
      //se nao tiver retorna somemte um categoryId
