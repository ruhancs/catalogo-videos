...
import { IRepository, ISearchableRepository } from "../../../domain/repository/repository_interface";
...
export abstract class InMemoryRepository<E extends Entity, EntityID extends ValueObject> implements IRepository<E, EntityID> {
    ...
    async update(entity: E): Promise<void> {
        const indexFounded = this.items.findIndex((item) => item.entity_id.equals(entity.entity_id))
        if (indexFounded === -1) {
    ...
    async delete(entity_id: EntityID): Promise<void> {
        const indexFounded = this.items.findIndex((item) => item.entity_id.equals(entity_id))
        if (indexFounded === -1) {
