...
export class VideoSearchParams extends SearchParams<VideoFilter> {
  ...
  protected set filter(value: VideoFilter | null) {
    //verificar se existe algum valor em value, se nao tiver nenhum valor value sera null
    const _value =
    ...
    //criar o filter verificando oque existe no value
    const filter = {
      //verificar se existe title no value, se existir inseri em filter
      ...(_value?.title && { title: `${_value?.title}` }),
      //verificar se existe categories id em value, se existir inseri em filter
      ...(_value?.categories_id &&
        _value.categories_id.length && {
          categories_id: _value.categories_id,
        }),
      //verificar se existe genres id em value, se existir inseri em filter
      ...(_value?.genres_id &&
        _value.genres_id.length && {
          genres_id: _value.genres_id,
        }),
      //verificar se existe cast members id em value, se existir inseri em filter
