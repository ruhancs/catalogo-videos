...
export class VideoSequelizeRepository implements IVideoRepository {
  ...
  //include das operacoes, para incluir dados das tabelas relacionadas
  relations_include = [
    'categories_id',
    'genres_id',
    'cast_members_id',
  ...
  async insert(entity: Video): Promise<void> {
    //transformar entidade em model, habilitar os campos relacinado e rodar no modo transacao
    await this.videoModel.create(VideoModelMapper.toModelProps(entity), {
      include: this.relations_include,
      transaction: this.uow.getTransaction(),
    });

    //adiciona no uow o agregado, para enviar eventos do agregado
  ...
  async update(entity: Video): Promise<void> {
    ...
    //destruir todos models relacionados ao video, todos destroy de uma vez só,
    //promisse all utilizado para uma transacao nao esperar a outra terminar
    //utilizado pois os campos relacionados de video nao tem nenhume relacao entre eles
    await Promise.all([
    ...
    //informacoes que se quer atualizar, que estao na entidade
    const {
      categories_id,
      genres_id,
      cast_members_id,
    ...
    //criar os models relacionados com video de uma vez só
    await Promise.all([
      ...image_medias.map((i) =>
  ...
  async delete(id: VideoId): Promise<void> {
    ...
    //adicionar no uow para enviar eventos do agregado
