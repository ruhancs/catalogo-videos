...
export class VideoModelMapper {
  static toEntity(model: VideoModel) {
    const {
      video_id: id,
      categories_id = [], //os relacionamentos podem vir undefined, entao habilitar como array vazio
      genres_id = [],
      cast_members_id = [],
      image_medias = [],
      audio_video_medias = [],
    ...
    //estrair de image_medias do videoModel, e encontrar o video_related_field = 'banner'
    const bannerModel = image_medias.find(
      (i) => i.video_related_field === 'banner',
    );

    //se tiver valor no bannerModel criar obj de valor Banner, se nao retorna null
    ...
    //encontrar o trailer em audio_video_medias do videoModel e encontrar video_related_field = 'trailer'
    const trailerModel = audio_video_medias.find(
      (i) => i.video_related_field === 'trailer',
    );

    //se tiver valor no trailerModel criar obj de valor Trailer, se nao retorna null
    ...
    //criar obj de valor Rating com o campo rating do videoModel
    const [rating, errorRating] = Rating.create(otherData.rating).asArray();

    //verificar se houve erro na criacao de rating
    if (errorRating) {
