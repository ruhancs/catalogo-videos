...
} from '../../../../../cast-member/infra/db/sequelize/cast-member-sequelize.repository';
...
import { Category } from '../../../../../category/domain/category.aggregat';
...
import { CategorySequelizeRepository } from '../../../../../category/infra/db/sequelize/category_sequelize.repository';
...
describe('VideoCategoryModel Unit Tests', () => {
  //iniciar instancia do sequelize com todos models
  setupSequelizeForVideo();

  test('table name', () => {
    //verificar o nome da tabela de category_video se foi inseriza na instancia do sequelize
    expect(VideoCategoryModel.tableName).toBe('category_video');
  });

  test('mapping props', () => {
    //pegar atributos da tabela pivo entre video e category (category_id e video_id)
    const attributesMap = VideoCategoryModel.getAttributes();
    //criar um array com as chaves dos atributos da tabela retorna um [category_id, video_id]
    const attributes = Object.keys(VideoCategoryModel.getAttributes());
    expect(attributes).toStrictEqual(['video_id', 'category_id']);

    //verificar coluna de video_id da tabela pivo entre video e category
